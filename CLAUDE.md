...
- That only reads correctly if the stored wall clock is the VIEWER's, so `parseICalTime` converts any **zone-qualified** value (a `TZID=` param or a trailing `Z`) into `eventZone` before stripping the zone. `zoneQualified()` makes the distinction
- **`eventZone` comes from `CALENDAR_TIMEZONE`, NOT from `TZ`, and defaults to UTC — never `time.Local`.** `cmd/server` resolves it at startup via `ical.SetEventZone` and logs the result; an unknown zone logs and keeps the default rather than silently moving every event. Keying off the process zone rendered the SAME event two hours apart in dev (`TZ=America/Los_Angeles`) and prod (`TZ` unset = UTC), and made test results depend on the machine. `TZ` also drives log timestamps, which must not shift the calendar. The repo default is generic UTC; each deployment sets its own zone (the household runs `CALENDAR_TIMEZONE=America/Los_Angeles`)
