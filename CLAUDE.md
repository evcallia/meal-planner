...
- `settings.packingShowChecked` (default true) hides packed items entirely; `POST /api/packing/lists/{id}/check-all {checked}` bulk-toggles with ONE undo entry (undo bulk-sets the majority value then patches the minority item-by-item)
