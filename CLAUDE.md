...
- **`eventZone` comes from `CALENDAR_TIMEZONE`, NOT from `TZ`.** `cmd/server` resolves it at startup via `ical.SetEventZone` and logs the result; an unknown zone logs and keeps the previous one rather than silently moving every event. It defaults to `time.Local` only when unset. This is deliberate: `docker-compose.yml` leaves `TZ` at UTC while `docker-compose-dev.yml` defaults it to `America/Los_Angeles`, so keying off the process zone rendered the SAME event two hours apart in dev and prod — and `TZ` also drives log timestamps, which nobody expects to shift the calendar
