...
- `settings.packingShowChecked` (default true) is the DEFAULT for completed-item visibility; `settings.packingShowCheckedOverrides[listId]` pins an individual list against it (`showCheckedOverrides[id] ?? showChecked`). The kebab's show/hide toggle writes an override for the ACTIVE list only; "Use this for all lists" sets the default to the current effective value and clears every override (offered only when it would change something). Hiding completed items hides them entirely; `POST /api/packing/lists/{id}/check-all {checked}` bulk-toggles with ONE undo entry (undo bulk-sets the majority value then patches the minority item-by-item)
