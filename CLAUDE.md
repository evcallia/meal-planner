...
- **Password auth**: `user_credentials` table (lowercase username = email, bcrypt hash, sub FK). `POST /api/auth/login/password` + self-service `POST /api/auth/password` (min 8 chars, keyed by session email) + `server -set-password <username>` CLI (password from `PASSWORD` env or stdin). `PASSWORD_AUTH_ENABLED=false` unregisters the routes (404). Unknown-user logins burn a dummy bcrypt compare (timing)
- **Identity linking (`resolveIdentity` in auth.go)**: session sub is canonical — at login, a case-insensitive email match in `users` wins over the provider's sub, so switching providers (or using password login) keeps all data; the new provider's sub never enters the DB. Oldest row (`last_seen ASC`) wins on duplicate emails. Applies to OIDC callback, password login, and dev-login
