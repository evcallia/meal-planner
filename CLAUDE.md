...
## Travel / Packing Lists (docs/packing-lists.md)
- A 5th tab ("Travel") combining the tracker's **multi-list, private-until-shared** model with the grocery tab's **sectioned checklist + chips**. Grocery "stores" become per-list **bags** (`PackingBag`) so two trips never share a bag vocabulary
...
- Bag progress (`utils/packing.ts: bagProgress`) is computed client-side: packed/total AND a percentage per bag + a total row, 100% bags struck through. Unassigned items group under a "None" row so the total is honest. "Hide bags" drops the per-bag rows AND the chips but KEEPS the overall total — that number is about the trip, not the bags
- Endpoints in `internal/app/packing.go` under `/api/packing`. All mutations broadcast `packing.updated` **per-user to the list's audience** (`BroadcastToUser`), never globally. Delete endpoints are idempotent; bag creates are idempotent by name (a replayed offline queue entry can't duplicate); a bag from another list is rejected 404
- Notifications/feed: category `travel`, pref key `notifyTravelEdits` (opt-in, default off), per-list mute via the SHARED `settings.listNotifyOverrides[listId].edits` namespace (ids are UUIDs, no clash with tracker). `push.QueueListEdit(category, …)` is the generalized batcher — `QueueTrackerEdit` now delegates to it. `logActivityScoped(category, detail, actor, *activityScope)` is the generalized feed writer (`logActivity` wraps it for tracker lists)
...
- Item-name suggestions have NO server table (bags are per-list): `usePacking.itemSuggestions` is derived from every list the user can see and remembers the bag by NAME, resolved against the active list. `ItemAutocomplete` takes `allowDelete={false}` there
...
- **Copy a section to another trip**: `usePacking.copySectionToList(fromListId, sectionId, toListId)` is composed from the existing *Core* primitives (which don't push undo), so it inherits optimistic updates + IndexedDB + offline queueing for free and is still ONE undo entry. Items arrive UNPACKED; bags carry across by NAME (reuse a matching bag in the target, else create it); a same-named section is merged into and item names already present are skipped, so copying twice is a no-op. `deleteSectionCore`/`deleteBagCore` were extracted so undo can reverse the whole copy. Surfaced via a per-section kebab (`ChecklistSectionCard`'s optional `headerActions` slot — Grocery passes nothing) plus a transient confirmation, since the change lands on a tab you're not looking at
- `PackingListsView.tsx` renders list tabs + bag-progress header + `ChecklistSectionCard`s; the share modal mirrors the tracker's. The kebab's "Rename & color" edits name + color in one save (one `updateList` call → one undo entry); trip tabs long-press-drag to reorder via `useTabReorder`
- **`hooks/useTabReorder.ts`** is the shared long-press tab-strip drag (250ms hold, `elementFromPoint` hit-test, edge auto-scroll, `justDraggedRef` swallowing the trailing click). `ListsView` and `PackingListsView` both use it; the hook only reports the finished order, so each caller decides what it means (Lists pins the synthetic Due Soon slot and keeps the viewed tab; Travel just persists)

## Shared checklist components (`components/ChecklistParts.tsx`)
- `ChecklistItemRow`, `ChecklistSectionCard`, `SectionCombobox`, `MenuRadioOption` and `NONE_STORE_ID` were extracted out of `GroceryListView` so Grocery and Travel share ONE implementation of swipe-to-delete, inline edit, drag handles and cross-section drop targets. `GroceryListView` re-exports `NONE_STORE_ID` for existing importers
...
- `ChecklistSectionCard` takes `visibleItems` (the parent decides filtering/ordering — grocery passes unchecked-only, packing passes its own order) and an optional `headerMeta` (grocery shows "N items", packing "3/7 packed"). `allowEditWhenChecked` is on for packing only

## Features (tab visibility)
- `settings.featureMeals/featurePantry/featureGrocery/featureLists/featureTravel` (default true, per-user synced) control which tabs exist — "Features" section in Settings (first section). PURELY visual: SSE handling + cache warming for hidden tabs keeps running
