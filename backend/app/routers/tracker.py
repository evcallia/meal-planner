...
def _audience(lst: TrackerList) -> set[str]:
    """Subs that can see this list: owner + everyone it's shared with."""
    return {lst.owner_sub} | {share.sub for share in lst.shares}
...
def _get_list(db: Session, list_id: UUID, sub: str, owner_only: bool = False) -> TrackerList:
    ...
    if lst.owner_sub != sub and not any(s.sub == sub for s in lst.shares):
...
async def list_lists(
    ...
    shared_ids = [s.list_id for s in db.query(TrackerShare).filter(TrackerShare.sub == sub).all()]
    shared = (
...
async def restore_list(
    ...
    for t in payload.tasks:
        ...
        for lg in t.logs:
            db.add(TrackerLog(
                task_id=task.id,
                done_at=lg.done_at or datetime.utcnow(),
