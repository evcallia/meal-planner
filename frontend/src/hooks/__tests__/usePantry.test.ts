...
describe('usePantry', () => {
  ...
  it('refreshes when a realtime pantry update arrives', async () => {
    mockGetPantryItems
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([{ id: '1', name: 'Rice', quantity: 1, updated_at: '2026-02-03T13:00:00Z' }]);
  ...
  it('hydrates from local storage only on the first load', async () => {
    ...
    mockGetPantryItems
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([{ id: 'server-1', name: 'Pasta', quantity: 1, updated_at: '2026-02-03T08:00:00Z' }])
      .mockResolvedValueOnce([]);
    mockCreatePantryItem.mockResolvedValue({ id: 'server-1', name: 'Pasta', quantity: 1, updated_at: '2026-02-03T08:00:00Z' });
