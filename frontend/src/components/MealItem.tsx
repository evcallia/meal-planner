...
export function MealItem({ html, itemized, onToggle, onTextClick, showHeader, showItemizedColumn = true }: MealItemProps) {
  const decodedHtml = decodeHtmlEntities(html);
  const linkedHtml = autoLinkUrls(decodedHtml);

  return (
    <div className={`flex items-start py-1.5 ${showItemizedColumn ? 'gap-3' : ''}`}>
      {showItemizedColumn && (
